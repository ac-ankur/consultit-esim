...
export default function EsimCountryCard({
  ...
  useEffect(() => {
    async function fetchImageFromJson() {
      try {
        const res = await fetch("/famous_places.json");
        const data = await res.json();
        const match = data.find(
          item => item.name.toLowerCase() === country?.countryName?.toLowerCase()
        );
        if (match?.path) {
          setImage(match.path);
        }
      } catch (err) {
        console.error("Failed to load image from JSON:", err);
        setImageError(true);
      } finally {
  ...
  return (
    <div
      ref={ref}
      className="group relative bg-gradient-to-br  shadow-lg shadow-emerald-500/50 from-white/80 via-white/60 to-white/40 backdrop-blur-xl border border-white/20 shadow-2xl rounded-3xl overflow-hidden transition-all duration-700 hover:scale-[1.02] hover:shadow-indigo-200/50 hover:border-indigo-200/30 cursor-pointer"
      style={{
        background:
          "linear-gradient(135deg, rgba(255,255,255,0.9) 0%, rgba(255,255,255,0.6) 50%, rgba(255,255,255,0.8) 100%)",
        backdropFilter: "blur(16px)",
    ...
    >
      ...
      <div className="relative z-10 ">
        ...
        <div className="p-6 text-center">
          <h3 className="text-xl font-bold text-gray-800 mb-1 tracking-wide group-hover:text-indigo-700 transition-colors duration-300 flex items-center justify-center min-w-0">
            <span className="block text-ellipsis whitespace-nowrap overflow-hidden text-[clamp(0.875rem, 3vw, 1.25rem)] max-w-full">
              {country?.countryName || "Country"}
            </span>
            <span className="text-sm font-normal text-gray-500 ml-1">eSIM</span>
          </h3>
          ...
          <button
            onClick={() => onBuy(country?.countryName?.toLowerCase())}
            className="block w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white px-4 py-3 rounded-lg font-medium shadow-md transition text-center"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
            <span
              style={{ cursor: "pointer" }}
